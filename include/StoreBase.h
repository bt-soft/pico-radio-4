...
class StoreBase {
 protected:
  ...
  virtual uint16_t performSave() {
    return StoreEepromBase<T>::save(getData(), 0, getClassName());
  }
  ...
  virtual uint16_t performLoad() {
    return StoreEepromBase<T>::load(getData(), 0, getClassName());
  }
 ...
 public:
  ...
  virtual void checkSave() {
    uint16_t currentCrc = Utils::calcCRC16(
        reinterpret_cast<const uint8_t*>(&getData()), sizeof(T));
   ...
   */
  uint16_t getCurrentCRC() const {
    return Utils::calcCRC16(reinterpret_cast<const uint8_t*>(&getData()),
                            sizeof(T));
  }
