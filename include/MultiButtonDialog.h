...
class MultiButtonDialog : public UIDialogBase, public ButtonsGroupManager<MultiButtonDialog> {
  ...
  protected:
    ...
    bool _autoCloseOnButtonClick = true;                // Automatikusan bezárja-e a dialógust gomb kattintáskor

    // Default choice gomb támogatás
  ...
  public: /**
           ...
           * @param defaultButtonIndex Az alapértelmezett (jelenlegi) gomb indexe, amely letiltva jelenik meg (-1 = nincs, alapértelmezett).
    ...
    MultiButtonDialog(UIScreen *parentScreen, TFT_eSPI &tft, const char *title, const char *message, const char *const *options, uint8_t numOptions,
                      ButtonClickCallback buttonClickCb = nullptr, bool autoClose = true, int defaultButtonIndex = -1, const Rect &bounds = {-1, -1, 0, 0},
    ...
    int getDefaultButtonIndex() const { return _defaultButtonIndex; } // Setter metódusok
