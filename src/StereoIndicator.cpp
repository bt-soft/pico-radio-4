...
/**
 * @brief StereoIndicator konstruktor
 */
StereoIndicator::StereoIndicator(TFT_eSPI &tft, const Rect &bounds, const ColorScheme &colors) : UIComponent(tft, bounds, colors) {
    isStereo = false;
    needsUpdate = true;
}
 ...
 */
void StereoIndicator::setStereo(bool stereo) {
    if (isStereo != stereo) {
        isStereo = stereo;
        needsUpdate = true;
        markForRedraw();
