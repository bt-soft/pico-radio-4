/**
 * @file SetupSi4735Screen.cpp
 * @brief Si4735 rádió chip beállítások képernyő implementációja
 *
 * Ez a fájl tartalmazza a SetupSi4735Screen osztály implementációját,
 * amely a Si4735 rádió chip specifikus beállításait kezeli.
 *
...
/**
 * @brief SetupSi4735Screen konstruktor
 *
 * @param tft TFT_eSPI referencia a kijelző kezeléséhez
 */
SetupSi4735Screen::SetupSi4735Screen(TFT_eSPI &tft) : SetupScreenBase(tft, "SETUP_SI4735") { layoutComponents(); }
 ...
 */
const char *SetupSi4735Screen::getScreenTitle() const { return "Si4735 Settings"; }
 ...
 */
void SetupSi4735Screen::populateMenuItems() {
 ...
 */
void SetupSi4735Screen::handleItemAction(int index, int action) {
    ...
    switch (si4735Action) {
        ...
        default:
            DEBUG("SetupSi4735Screen: Unknown action: %d\n", action);
            break;
    }
}
 ...
 */
String SetupSi4735Screen::decodeFFTConfig(float value) {
    if (value == -1.0f)
        return "Disabled";
    else if (value == 0.0f)
        return "Auto Gain";
 ...
 */
void SetupSi4735Screen::handleSquelchBasisDialog(int index) {
 ...
 */
void SetupSi4735Screen::handleFFTConfigDialog(int index, bool isAM) {
            ...
            switch (buttonIndex) {
                ...
                case 2: // Manual Gain
                {
                    ...
                    auto gainDialog = std::make_shared<ValueChangeDialog>(
                        this, this->tft, (String(title) + " - Manual Gain").c_str(), "Set gain factor (0.1 - 10.0):", tempGainValuePtr.get(), 0.1f, 10.0f, 0.1f, nullptr,
 ...
 */
void SetupSi4735Screen::handleToggleItem(int index, bool &configValue) {
